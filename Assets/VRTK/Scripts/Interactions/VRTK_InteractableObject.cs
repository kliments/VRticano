...
namespace VRTK
{
    ...
    public class VRTK_InteractableObject : MonoBehaviour
    {
        ...
        public virtual void Grabbed(GameObject currentGrabbingObject)
        {
            ...
            {
                GetComponent<explosionEffect>().objectIsGrabbed = true;
            }
            ToggleEnableState(true);
        ...
        protected virtual void Update()
        {
            ...
            {
                leftController = cameraRig.transform.GetChild(0).gameObject;
            }
            if (rightController == null)
            {
                rightController = cameraRig.transform.GetChild(1).gameObject;
            }
            if (isGrabbed)
            {
                leftGrip = leftController.GetComponent<SteamVR_TrackedController>().gripped;
                leftIndex = (int)leftController.GetComponent<SteamVR_TrackedController>().controllerIndex;
                rightGrip = rightController.GetComponent<SteamVR_TrackedController>().gripped;
                rightIndex = (int)rightController.GetComponent<SteamVR_TrackedController>().controllerIndex;
                if (leftGrip == true && rightGrip == true)
                {
                    SteamVR_Controller.Input(leftIndex).TriggerHapticPulse(3999);
                    SteamVR_Controller.Input(rightIndex).TriggerHapticPulse(3999);
                }

                else if (leftController.GetComponent<SteamVR_TrackedController>().gripped == true)
                {
                    SteamVR_Controller.Input(leftIndex).TriggerHapticPulse(3999);
                }

                else if (rightController.GetComponent<SteamVR_TrackedController>().gripped == true)
                {
                    SteamVR_Controller.Input(rightIndex).TriggerHapticPulse(3999);
                }
            }
